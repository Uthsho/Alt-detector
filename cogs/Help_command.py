...
class Help_command(commands.Cog):
  ...
  async def setchannel(self, ctx):
  	embed = discord.Embed(title = "setchannel [#channel/channel_id]", colour = random.choice(ListColours), description = "This will be the channel where the bot logs the member joins.")
  	embed.set_thumbnail(url = self.bot.user.avatar_url)
  	embed.add_field(name = 'Note: DO NOT include the [] brackets, make sure the bot has permissions to send message in the designated channel.', value = 'For example:\nad!setchannel #abcd or\nad!setchannel self.')
  	await ctx.send(embed = embed)

  @help.command()
  async def setage(self, ctx):
  	embed = discord.Embed(title = "setage [days]", colour = random.choice(ListColours), description = "If the age of the account joining is less than the number you provide in the parameter days, it will be identified as an alt.")
  	embed.set_thumbnail(url = self.bot.user.avatar_url)
  	embed.add_field(name = 'Note: DO NOT include the [] brackets, the default value is 7.', value = 'For example:\nad!setage 15.')
  	await ctx.send(embed = embed)

  @help.command()
  async def setnotify(self, ctx):
  	embed = discord.Embed(title = "setnotify [@role/role_id]", colour = random.choice(ListColours), description = "This will be the which the bot would ping when suspicious members join.")
  	embed.set_thumbnail(url = self.bot.user.avatar_url)
  	embed.add_field(name = "Note: DO NOT include the [] brackets, make sure the bots role is above the role to be pinged", value = 'For example:\nad!setnotify @role or\nad!setrole self.')
  	await ctx.send(embed = embed)

  @help.command()
  async def fetchalts(self, ctx):
  	embed = discord.Embed(title = "fetchalts [days]", colour = random.choice(ListColours), description = "Use this command to find accounts younger than the number of days you provide in the `[days]` parameter.")
  	embed.set_thumbnail(url = self.bot.user.avatar_url)
  	embed.add_field(name = "Note: DO NOT include the [] brackets.\nTo prevent spam, the `[days]` parameter cannot be more than 90 days", value = 'For example:\nad!fetchalts 3.')
  	await ctx.send(embed = embed)

  @help.command()
  async def rolemen(self, ctx):
  	embed = discord.Embed(title = "rolemen", colour = random.choice(ListColours), description = "Use this command to find a list of roles which can be mentioned by anyone.")
  	embed.set_thumbnail(url = self.bot.user.avatar_url)
  	embed.add_field(name = "Sub-commands:", value = "`true`: Returns a list of mentionable roles.\n`false`: Returns a list of unmentionable roles.")
  	embed.add_field(name = "For example:", value = 'ad!rolemen\nad!rolemen true\nad!rolemen false', inline = False)
  	await ctx.send(embed = embed)
